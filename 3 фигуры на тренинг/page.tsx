...
const getRandomShapes = () => {
    ...
    return shapesCopy.sort(() => 0.5 - Math.random()).slice(0, 3);
};

const getRandomGestures = () => {
    const gesturesCopy = [...GESTURES];
    return gesturesCopy.sort(() => 0.5 - Math.random()).slice(0, 3);
};

const generateShapeSequence = (selectedShapes: any) => {
    const targetShapes = [
        ...Array(5).fill(selectedShapes[0]),
        ...Array(5).fill(selectedShapes[1]),
        ...Array(5).fill(selectedShapes[2]),
    ];
    const fillerShapes = [];
    while (fillerShapes.length < 15) {
...
const FifthTest: React.FC = () => {
    ...
    const startCapture = () => {
        ...
        setShapeGestureDescription({
            ...
            shape3: selectedShapes[2],
            gesture3: selectedGestures[2]
        });
        setCapturing(true);
    ...
    const captureSequence = async (shapesGestures: { [key: string]: string }) => {
        ...
        for (let i = 0; i < shapeSequence.length; i++) {
            ...
            // Check if we have shown all gestures 5 times
            if (gestureOrder.length === 15) {
                break;
            }
        }
    ...
    const sendToBackend = async (gestureOrder: string[], images: string[]) => {
        ...
        const response = await fetch('http://195.49.210.50:8872/classify-strict-db', {
            method: 'POST',
            body: formData,
        });
        ...
        <div className="text-center pt-5 min-h-screen">
            ...
            {showDescription && (
                <div className="text-xl mx-auto p-4 bg-white rounded-lg shadow-md text-gray-700 max-w-xl">
                    <p>Запомните 3 жеста, привязанные к фигурам указанного цвета. Показывайте эти жесты, когда вы увидите соотвестующие фигуры на экране.</p>
                </div>
            )}
            <button
                ...
                <div className="flex flex-col items-center mt-4">
                    <div className="flex items-center mb-4">
                        <Image src={`/figures/${SHAPE_IMAGES[shapeGestureDescription.shape1]}`}
                               alt={shapeGestureDescription.shape1} width={100} height={100}/>
                        <span className="mx-2"> - </span>
                        <Image src={`/gestures/${GESTURE_IMAGES[shapeGestureDescription.gesture1]}`}
                               alt={shapeGestureDescription.gesture1} width={100} height={100}/>
                    </div>
                    <div className="flex items-center mb-4">
                        <Image src={`/figures/${SHAPE_IMAGES[shapeGestureDescription.shape2]}`}
                               alt={shapeGestureDescription.shape2} width={100} height={100}/>
                        <span className="mx-2"> - </span>
                        <Image src={`/gestures/${GESTURE_IMAGES[shapeGestureDescription.gesture2]}`}
                               alt={shapeGestureDescription.gesture2} width={100} height={100}/>
                    </div>
                    <div className="flex items-center">
                        <Image src={`/figures/${SHAPE_IMAGES[shapeGestureDescription.shape3]}`}
                               alt={shapeGestureDescription.shape3} width={100} height={100}/>
                        <span className="mx-2"> - </span>
                        <Image src={`/gestures/${GESTURE_IMAGES[shapeGestureDescription.gesture3]}`}
                               alt={shapeGestureDescription.gesture3} width={100} height={100}/>
                    </div>
                </div>
            )}
            {currentShape &&
                <Image src={`/figures/${SHAPE_IMAGES[currentShape]}`} alt={currentShape} width={150} height={150}
                       className="mx-auto"/>}
            {results && (
                <div
