    ...
    useEffect(() => {
        if (id) {
            const fetchRecommendations = async () => {
                try {
                    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/results/${id}`);
                    if (response.ok) {
            ...
            <div className="w-full max-w-2xl mx-auto bg-white shadow-lg rounded-lg p-6 space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    {loading && (
                        <div className="flex items-center space-x-2">
                            <div
                                className="loader ease-linear rounded-full border-8 border-t-8 border-gray-200 h-8 w-8"></div>
                ...
                {user && !loading && (
                    <div className="space-y-4">
                        ...
                        <p className="text-gray-700">
                            <span className="font-bold">Предполагаемый вердикт:</span> {user.predicted_diagnosis}
                        </p>
                    </div>
                )}
